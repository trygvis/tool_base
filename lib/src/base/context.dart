...
class AppContext {
  ...
  dynamic _generateIfNecessary(Type type, Map<Type, Generator> generators) {
    ...
    return _values.putIfAbsent(type, () {
      final List<Type> checks = _reentrantChecks ??= <Type>[];

      final int index = checks.indexOf(type);
      if (index >= 0) {
        // We're already in the process of trying to generate this type.
        throw ContextDependencyCycleException._(
            UnmodifiableListView<Type>(checks.sublist(index)));
      }
  ...
  T? get<T>() {
    Object? value = _generateIfNecessary(T, _overrides);
    value ??= _parent?.get<T>();
